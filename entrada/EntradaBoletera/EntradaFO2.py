...
# -----------------------------------
    def ExpedirRfid(self):    
        ...
        self.labelframe1=ttk.LabelFrame(self.pagina1, text=" ") ##Contiene Imagen de la Plaza
        self.labelframe1.grid(column=1, row=0, padx=0, pady=0)
        self.Adentroframe=ttk.LabelFrame(self.pagina1, text=" ") ##Relog y Fecha
        self.Adentroframe.grid(column=2, row=3, padx=0, pady=0)
        ...
        self.loopDet.grid(column=2, row=0, padx=0, pady=0)
        self.BotDet = ttk.Label(self.pagina1, text="Boton", width = 17)#, background = '#CCC')
        self.BotDet.grid(column=2, row=12, padx=0, pady=0)
        
        #self.Reloj = ttk.Label(self.pagina1, text="Hora y fecha", width = 10, background = '#FD6')
        #self.Reloj.grid(column=0, row=6, padx=0, pady=0)

        self.Reloj = ttk.Label(self.Adentroframe, text="Reloj") #Creación del Label
        self.Reloj.config(width =10)
        self.Reloj.config(background="white") #Cambiar color de fondo
        self.Reloj.config(font=('Arial', 42)) #Cambiar tipo y tamaño de fuente 102
        self.Reloj.grid(column=1, row=4, padx=0, pady=0)   
        
        self.mi_reloj = ttk.Label(self.Adentroframe, text="Reloj") #Creación del Label
        self.mi_reloj.config(width =10)
        self.mi_reloj.config(background="white") #Cambiar color de fondo
        self.mi_reloj.config(font=('Arial', 42)) #Cambiar tipo y tamaño de fuente 102
        self.mi_reloj.grid(column=1, row=9, padx=0, pady=0)        
        ...
        self.scrolledtext=st.ScrolledText(self.labelframe1, width=15, height=8)
        self.scrolledtext.grid(column=1,row=0, padx=4, pady=4)
        #self.Autdentro=tk.Button(self.Adentroframe, text="Boletos sin Cobro", command=self.Autdentro, width=12, height=1, anchor="center")
        ...
        #self.botonPent=tk.Button(self.Adentroframe, text="DeclararlaEnt", command= self.check_inputs, width=12, height=1, anchor="center")
        ...
        self.entryNumTarjeta=tk.Entry(self.labelframe3, width=20, textvariable=self.NumTarjeta)
        self.entryNumTarjeta.bind('<Return>',self.Pensionados)#con esto se lee automatico y se va a consultar
        self.entryNumTarjeta.grid(column=1, row=2, padx=4, pady=4)
    ...
    def check_inputs(self):
        ...
        if io.input(loop):
                ...
                #time.sleep (1)
                tarjeta=str(self.entryNumTarjeta.get(),)
                if len(tarjeta) == 10:
                    mb.showinfo("Información", "ENTRO")   
                if io.input(boton):
        ...
        now1 = now [:10]
        now2 = now [10:19]       
        self.Reloj.config(text=now1)            
        self.mi_reloj.config(text=now2)    
...
#$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$impresion fin$$$$$$$$$$$$$$$$        
    ...
    def Pensionados(self,event):
       numtarjeta=str(self.NumTarjeta.get(), )
       if len(numtarjeta) == 0:
            ...
            if len(Existe) == 0 :
                ...
                for fila in respuesta:
                    ...
                    if Estatus == 'Adentro' :
                        ...
                        mb.showwarning("IMPORTANTE", "SIN VIGENCIA ACTIVA: Pensionado sin pago, favor de realizar el pago")
                        self.NumTarjeta.set("")               
                        self.entryNumTarjeta.focus()
                        return False

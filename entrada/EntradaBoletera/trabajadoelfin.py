...
##out3 =                      #Tierra,Salida Detector de barrera abierta
loop = 16                     #gpio16,pin36,entrada loop detector de autos
SenBoleto = 20                #gpio20,pin38, entrada sensor boleto detecta papel
##Salidas
out2 = 6                      #gpio6,pin31,Salida indicador boton
barrera = 13                  #gpio13,pin33,Salida barrera manda abrir
out1 = 19                     #gpio19,pin35,Salida indicador loop
IndSenBoleto = 26             #gpio26,pin37,Indicador Sensor de Boleto detecta papel           
...
io.setup(IndSenBoleto,io.OUT)  
line=''
io.output(barrera,0)
io.output(out1,0)
io.output(out2,0)
io.output(IndSenBoleto,0)
BanLoop =0 #En 0 es no hay auto, y en 1 es hay auto 
BanBoton=0 #En 0 Botón suelto, y en 1 botón presionado
BanSenBoleto=0 #no ha tomado el boleto
BanImpresion=0 #No ha impreso
BanBarrera=0 #Barrera nunca abierta
...
# -----------------------------------
    def ExpedirRfid(self):    
        ...
        self.StaImp = ttk.Label(self.pagina1, text="Estatus Impresion", width = 17)#, background = '#CCC')
        self.StaImp.grid(column=1, row=14, padx=0, pady=0)
    ...
    def SenBoleto(self): #Detecta presencia del Boleto
        global BanSenBoleto
        if io.input(SenBoleto): 
                print('siente boleto')
                io.output(IndSenBoleto,1)#con un "1" se apaga el led 
                BanSenBoleto = 1
        else:   
                print('no siente boleto ')
                io.output(IndSenBoleto,0)                              
                BanSenBoleto = 0
                print(str(io.input(SenBoleto)))
    ...
    def check_inputs(self):
        ...
        if BanLoop == 1:
                ...
                if (BanBoton == 1):#BanBoton == 1 esta oprimido el boton
                    BanPasoXUno =1
                    self.BotDet.config(text = "Boton Oprimido",background="green")
                else:    
                    BanPasoXUno=0
                    self.BotDet.config(text = "Boton suelto",background="#CCC")
                if BanImpresion == 0 and BanPasoXUno == 1:
                   print('imprimir')
                   BanImpresion=1
                else:
                   print('ya imprimio')
                if BanSenBoleto == 0 and BanImpresion == 1 and BanBarrera ==0 :
                   print('abrir barrera')
                   self.SenBol.config(text = "No siente boleto ", background = '#CCC') #
                   BanBarrera = 1
                else:   
                   print('no abrir barrera esperar quiten boleto')
                   self.SenBol.config(text = "siente boleto ", background = 'green' ) # 
                
                #self.StaImp.config(text = "Estatus impresion",background="#CCC")                
            ...
            #if BanBarrera == 1 and BanSenBoleto == 0 and BanLoop == 1:# Barrera Cerrada y no hay boleto BanImpresion == 1 and BanSenBoleto == 0:
                ...
                #BanBarrera = 0
                     ...
                     #io.output(out2,0)             
                     #print('imprimir')
                     #self.StaImp.config(text = "Imprime Boleto",background="red")
                     #self.agregarRegistroRFID()
                     #BanImpresion = 1
                     #if BanSenBoleto == 1:
                        #print("Hay boleto y no puedo abrir pluma")
                        #BanBarrera = 1
                     #else:
                        #print("No hay boleto, abre pluma")
                        
                        #io.output(out3,1)#con un "1" se apaga el led
                        #io.output(barrera,1)#con un "0" abre la barrera
                        #time.sleep (1)
                        #io.output(barrera,0)
                        #BanBarrera = 0 #Ya abrio la Barrera
                        #self.SenBol.config(text = "siente boleto ")                    
                #else:
                     #print ('no puede imprimir')
                     #print(str(BanSenBoleto))
                     #self.StaImp.config(text = "NO puede imprimir",background="red")
                     #if BanImpresion == 1 and BanSenBoleto == 0 and BanBarrera == 1:
                        #io.output(barrera,1)#con un "0" abre la barrera
                        #time.sleep (1)
                        #io.output(barrera,0)                             
                        #BanBarrera = 0 #Ya abrio la barrera
